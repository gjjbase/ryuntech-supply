    ...
    <el-row :gutter="20">
      ...
      <el-col :span="8">
        <el-card class="box-card">
          <div slot="header">
            <div class="title-box">
              ...
              <el-tooltip content="同步菜单权限数据" placement="top">
                <el-button style="font-size: 25px;" type="text" icon="el-icon-refresh" circle @click="handleSyncMenuPermissionData" />
              </el-tooltip>
            </div>
          ...
          <el-input v-model="filterMenuPermText" class="mgb-15" :placeholder="filterPlaceholderText" />
          <el-tree
            ref="menuPermTreeRef"
      ...
      <el-col :span="8">
        <el-card class="box-card">
          ...
          <el-input v-model="filterButtonPermText" class="mgb-15" :placeholder="filterPlaceholderText" />
          <el-tree
          ...
          >
            <span slot-scope="{ node, data }" class="custom-tree-node">
              <span>
                <span class="mgl-10">{{ data.pname }}</span>
                <span class="mgl-10 tips-text">{{ data.pval }}</span>
                <el-tag v-if="data.ptype==permType.MENU" class="mgl-10" type="success" size="mini">菜单</el-tag>
                <el-tag v-else-if="data.ptype==permType.BUTTON" class="mgl-10" type="warning" size="mini">按钮</el-tag>
              </span>
      ...
      <el-col :span="8">
        <el-card class="box-card">
          <div slot="header">
            <div class="title-box">
              ...
              <el-tooltip content="同步接口权限数据" placement="top">
                <el-button style="font-size: 25px;" type="text" icon="el-icon-refresh" circle @click="handleSyncApiPermissionData" />
              </el-tooltip>
            </div>
          ...
          <el-input v-model="filterApiPermText" class="mgb-15" :placeholder="filterPlaceholderText" />
          <el-tree
            ref="apiPermTreeRef"
            :filter-node-method="filterNode"
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="30%">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="top">
        <el-form-item label="权限名" prop="pname">
          <el-input v-model="temp.pname" placeholder="例如：用户管理、添加用户" />
        </el-form-item>
        <el-form-item label="权限值" prop="pval">
          <el-input v-model="temp.pval" placeholder="已自动加上前缀'b:'，您只需填剩余部分，如：user:manage、user:add" :disabled="dialogStatus=='updateButton'">
            <template v-if="dialogStatus=='addButton'" slot="prepend">{{ btnPermPrefix }}</template>
          </el-input>
        ...
        <el-form-item label="权限类型" prop="ptype">
          <el-select v-if="dialogStatus=='updateButton'||dialogStatus=='addButton'" v-model="temp.ptype" :disabled="true">
            <el-option label="按钮" :value="permType.BUTTON" />
          </el-select>
        </el-form-item>
      </el-form>
