<template>
  <div class="app-container">
    <el-card>
      <div>
        <el-input v-model="search.order.memberName" style="width: 200px;" placeholder="客户姓名" />
        <el-input v-model="search.order.partnerId" style="width: 200px;" placeholder="推荐人" />
        <el-select v-model="search.paymentStatus" placeholder="请选择佣金状态">
          <!--          结算状态(03-可结算，01已结算，02结算中)-->
          <el-option label="结算中" value="02" />
          <el-option label="已结算" value="01" />
        </el-select>
      ...
      <el-table v-loading="listLoading" border :data="list" element-loading-text="Loading" border fit highlight-current-row>
        ...
        <el-table-column align="center" prop="memberName" label="客户名称" width="150">
          <template v-if="scope.row.order!=null" slot-scope="scope">
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.paymentStatus==='02'" type="primary" size="mini" @click="handleUp(scope.row.paymentSystemId)">确认结算</el-button>
          </template>
        </el-table-column>
      </el-table>
